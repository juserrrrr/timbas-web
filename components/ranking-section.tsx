...
export function RankingSection({
  ...
  const selectedServerId = searchParams.get('serverId') || initialServerId;
        ...
        <div className="w-full md:w-auto">
          <Select
            ...
            onValueChange={(serverId) =>
              router.push(`/dashboard/ranking?serverId=${serverId}`)
            }
          >
              ...
              {servers.map((server) => (
                ...
                >
                  <div className="flex items-center gap-2">
                    <img
                      src={server.icon || '/user-avatar.jpg'}
                      alt={server.name}
      ...
      {players.length === 0 && (
        <div className="flex h-96 flex-col items-center justify-center gap-4 text-white">
          ...
          <p className="text-lg text-gray-400">
            Nenhum jogador no ranking deste servidor ainda.
          </p>
        </div>
      )}
      ...
      {players.length > 0 && (
        <>
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
            {/* 2nd Place */}
            <Card
              className={`order-2 border p-6 backdrop-blur-sm md:order-1 ${getMedalBg(
                2
              )}`}
            >
              ...
              <div className="mb-4 flex items-center gap-3">
                <Avatar className="h-16 w-16 border-2 border-gray-400">
                  <AvatarImage
                    src={topThree[1].avatar || '/user-avatar.jpg'}
                    alt={topThree[1].name}
                  />
                ...
                <div className="mb-4 flex items-center gap-3">
                  <div className="rotating-border-glow rounded-full">
                    <Avatar className="h-20 w-20 border-4 border-yellow-400">
                      <AvatarImage
                        src={topThree[0].avatar || '/user-avatar.jpg'}
                        alt={topThree[0].name}
                      />
            ...
            {/* 3rd Place */}
            <Card
              className={`order-3 border p-6 backdrop-blur-sm ${getMedalBg(3)}`}
            >
              <div className="mb-4 flex items-center justify-between">
              ...
              <div className="mb-4 flex items-center gap-3">
                <Avatar className="h-16 w-16 border-2 border-amber-600">
                  <AvatarImage
                    src={topThree[2].avatar || '/user-avatar.jpg'}
                    alt={topThree[2].name}
                  />
          ...
          <Card className="border-gray-800 bg-gray-900/50 backdrop-blur-sm">
                        ...
                        <td className="p-4">
                          <div className="flex items-center gap-3">
                            <Avatar className="h-10 w-10">
                              <AvatarImage
                                src={player.avatar || '/user-avatar.jpg'}
                                alt={player.name}
                              />
            ...
            <div className="flex items-center justify-between border-t border-gray-800 p-4">
              <div className="text-sm text-gray-400">
                ...
                {Math.min(endIndex, remainingPlayers.length)} de{' '}
                {remainingPlayers.length} jogadores
              </div>
              <div className="flex items-center gap-2">
                <Button
                  variant="outline"
