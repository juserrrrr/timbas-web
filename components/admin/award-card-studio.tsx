...
function textFitScale(text: string, family: string, weight: number, size: number, width: number, enabled: boolean) {
  if (!enabled || typeof document === "undefined") return 1
  const canvas = document.createElement("canvas")
  const context = canvas.getContext("2d")
  if (!context) return 1
  context.font = `${weight} 100px "${family}", Impact, sans-serif`
  const naturalWidth = context.measureText(text.toUpperCase()).width / 100 * size
...
export function AwardCardStudio() {
  ...
  const nickScale = textFitScale(nickname || "Jogador", family, weight, layout.nickSize, layout.textWidth, layout.nickAutoFit)
  const statScale = textFitScale(achievement || "0", family, weight, layout.statSize, layout.textWidth, layout.statAutoFit)
  ...
  return <div className="mt-5 grid min-w-0 gap-5 lg:grid-cols-[370px_minmax(420px,1fr)]">
    <div className="min-w-0 space-y-4 rounded-2xl border border-white/[0.07] bg-black/25 p-4">
      ...
      <div className="space-y-2 rounded-xl border border-white/[0.07] bg-black/25 p-3">
        ...
        {selected !== "qr" && <><RangeControl label="Largura campo" value={layout.textWidth} min={0.2} max={0.7} onChange={(textWidth) => updateLayout({ textWidth })} /><label className="mt-2 flex items-center justify-between rounded-lg border border-emerald-500/20 bg-emerald-500/[0.05] px-2.5 py-2 text-[10px] font-bold text-emerald-200">Ajustar texto ao limite<input type="checkbox" checked={selected === "nick" ? layout.nickAutoFit : layout.statAutoFit} onChange={(event) => { if (selected === "nick") updateLayout({ nickAutoFit: event.target.checked }); else updateLayout({ statAutoFit: event.target.checked }); returnFocusToCanvas() }} className="accent-emerald-400" /></label></>}
      </div>
    ...
    <div className="min-w-0"><div className="mb-2 flex items-center justify-between text-[10px] font-black uppercase tracking-[0.16em] text-gray-500"><span>Setas 0,01 · Shift 0,1 · Ctrl 1,0</span><span>PNG em 2×</span></div>
      <div ref={previewRef} tabIndex={0} onKeyDown={nudgeSelected} onPointerMove={move} onPointerUp={() => { dragging.current = null }} onPointerCancel={() => { dragging.current = null }} className="relative mx-auto aspect-[4/5] w-full max-w-[620px] touch-none overflow-hidden rounded-2xl bg-black shadow-2xl ring-1 ring-white/10 outline-none focus:ring-2 focus:ring-emerald-400/60" style={{ containerType: "inline-size" }}>
        ...
        <button type="button" onPointerDown={(event) => beginDrag(event, "nick")} className={`absolute flex cursor-move items-center justify-center border border-dashed px-1 text-center uppercase leading-none ${selectionFrames && selected === "nick" ? "border-emerald-300/80 bg-emerald-300/5" : "border-transparent"}`} style={{ left: `${layout.nickX * 100}%`, top: `${layout.nickY * 100}%`, width: `${layout.textWidth * 100}%`, transform: "translate(-50%,-50%)" }}><span className="inline-block whitespace-nowrap" style={{ color: "#f4f6f8", fontFamily: family, fontWeight: weight, fontSize: `${layout.nickSize * 100}cqw`, transform: `scale(${nickScale})` }}>{nickname || "Jogador"}</span></button>
        <button type="button" onPointerDown={(event) => beginDrag(event, "stat")} className={`absolute flex cursor-move items-center justify-center border border-dashed px-1 text-center uppercase leading-none ${selectionFrames && selected === "stat" ? "border-emerald-300/80 bg-emerald-300/5" : "border-transparent"}`} style={{ left: `${layout.statX * 100}%`, top: `${layout.statY * 100}%`, width: `${layout.textWidth * 100}%`, transform: "translate(-50%,-50%)" }}><span className="inline-block whitespace-nowrap" style={{ color: award.highlight, fontFamily: family, fontWeight: weight, fontSize: `${layout.statSize * 100}cqw`, transform: `scale(${statScale})` }}>{achievement || "0"}</span></button>
