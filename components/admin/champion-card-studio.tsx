    ...
    <div className="grid min-w-0 gap-5 lg:grid-cols-[390px_minmax(420px,1fr)]">
      <div className="min-w-0 space-y-4 rounded-2xl border border-white/[0.07] bg-black/25 p-4">
        ...
        <div className="space-y-2 rounded-xl border border-white/[0.07] bg-black/25 p-3">
          {selected === "championTitle" && <><RangeControl label="Horizontal" value={layout.championTitleX} min={0.15} max={0.85} onChange={(championTitleX) => update({ championTitleX })} /><RangeControl label="Vertical" value={layout.championTitleY} min={0.55} max={0.72} onChange={(championTitleY) => update({ championTitleY })} /><RangeControl label="Tamanho" value={layout.championTitleSize} min={0.012} max={0.06} onChange={(championTitleSize) => update({ championTitleSize })} /><RangeControl label="Largura" value={layout.championTitleWidth} min={0.08} max={0.5} onChange={(championTitleWidth) => update({ championTitleWidth })} /></>}
