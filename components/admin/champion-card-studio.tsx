...
type EditableElement = "championTitle" | "team" | "tournament" | "rosterTitle" | "roster" | "qr"
...
export function ChampionCardStudio() {
  ...
  function move(event: ReactPointerEvent<HTMLDivElement>) {
    ...
    if (dragging.current === "championTitle") update({ championTitleX: x, championTitleY: y })
    if (dragging.current === "team") update({ teamX: x, teamY: y })
    if (dragging.current === "tournament") update({ tournamentX: x, tournamentY: y })
    if (dragging.current === "rosterTitle") update({ rosterTitleX: x, rosterTitleY: y })
  ...
  function nudgeSelected(event: ReactKeyboardEvent<HTMLDivElement>) {
    ...
    if (selected === "championTitle") update({ championTitleX: clamp(layout.championTitleX + dx, 0.15, 0.85), championTitleY: clamp(layout.championTitleY + dy, 0.55, 0.72) })
    ...
    if (selected === "rosterTitle") update({ rosterTitleX: clamp(layout.rosterTitleX + dx, 0.1, 0.75), rosterTitleY: clamp(layout.rosterTitleY + dy, 0.68, 0.82) })
  ...
  return <div className="mt-8 border-t border-amber-400/15 pt-6">
    ...
    <div className="grid min-w-0 gap-5 lg:grid-cols-[390px_minmax(420px,1fr)]">
      <div className="min-w-0 space-y-4 rounded-2xl border border-white/[0.07] bg-black/25 p-4">
        ...
        <div><p className="mb-2 text-[10px] font-black uppercase tracking-[0.16em] text-gray-500">Elemento para ajustar</p><div className="grid grid-cols-3 gap-2">{([{ key: "championTitle", label: "Campeão", icon: Trophy }, { key: "team", label: "Time", icon: Type }, { key: "tournament", label: "Copa", icon: Trophy }, { key: "rosterTitle", label: "Título elenco", icon: Type }, { key: "roster", label: "Nomes", icon: Users }, { key: "qr", label: "QR", icon: QrCode }] as const).map((item) => <button key={item.key} type="button" onClick={() => selectElement(item.key)} className={`flex h-10 items-center justify-center gap-1 rounded-xl border text-[9px] font-black ${selected === item.key ? "border-emerald-400 bg-emerald-400/10 text-emerald-300" : "border-white/10 text-gray-500"}`}><item.icon className="h-3 w-3" />{item.label}</button>)}</div></div>
        <div className="space-y-2 rounded-xl border border-white/[0.07] bg-black/25 p-3">
          {selected === "championTitle" && <><RangeControl label="Horizontal" value={layout.championTitleX} min={0.15} max={0.85} onChange={(championTitleX) => update({ championTitleX })} /><RangeControl label="Vertical" value={layout.championTitleY} min={0.55} max={0.72} onChange={(championTitleY) => update({ championTitleY })} /><RangeControl label="Tamanho" value={layout.championTitleSize} min={0.012} max={0.06} onChange={(championTitleSize) => update({ championTitleSize })} /></>}
          ...
          {selected === "rosterTitle" && <><RangeControl label="Horizontal" value={layout.rosterTitleX} min={0.1} max={0.75} onChange={(rosterTitleX) => update({ rosterTitleX })} /><RangeControl label="Vertical" value={layout.rosterTitleY} min={0.68} max={0.82} onChange={(rosterTitleY) => update({ rosterTitleY })} /><RangeControl label="Tamanho" value={layout.rosterTitleSize} min={0.009} max={0.04} onChange={(rosterTitleSize) => update({ rosterTitleSize })} /><RangeControl label="Largura" value={layout.rosterTitleWidth} min={0.1} max={0.35} onChange={(rosterTitleWidth) => update({ rosterTitleWidth })} /></>}
      ...
      <div className="min-w-0"><div className="mb-2 flex items-center justify-between text-[10px] font-black uppercase tracking-[0.16em] text-gray-500"><span>Setas 0,01 · Shift 0,1 · Ctrl 1,0</span><span>2240 × 2800</span></div>
        <div ref={previewRef} tabIndex={0} onKeyDown={nudgeSelected} onPointerMove={move} onPointerUp={() => { dragging.current = null }} onPointerCancel={() => { dragging.current = null }} className="relative mx-auto aspect-[4/5] w-full max-w-[620px] touch-none overflow-hidden rounded-2xl bg-black shadow-2xl ring-1 ring-white/10 outline-none focus:ring-2 focus:ring-emerald-400/60" style={{ containerType: "inline-size" }}>
          ...
          <button type="button" onPointerDown={(event) => beginDrag(event, "championTitle")} className={`absolute z-10 flex cursor-move items-center justify-center border border-dashed px-2 font-teko font-semibold leading-none text-[#f3cc72] ${selectionFrames && selected === "championTitle" ? "border-emerald-300/80 bg-emerald-300/5" : "border-transparent"}`} style={{ left: `${layout.championTitleX * 100}%`, top: `${layout.championTitleY * 100}%`, fontSize: `${layout.championTitleSize * 100}cqw`, transform: "translate(-50%,-50%)" }}>CAMPEÃO</button>
          ...
          <button type="button" onPointerDown={(event) => beginDrag(event, "rosterTitle")} className={`absolute z-10 flex cursor-move items-center justify-center whitespace-nowrap border bg-[#050508]/95 px-[0.6cqw] font-teko font-semibold leading-none text-[#d9bc78] ${selectionFrames && selected === "rosterTitle" ? "border-emerald-300/80" : "border-[#c9a85d]/50"}`} style={{ left: `${layout.rosterTitleX * 100}%`, top: `${layout.rosterTitleY * 100}%`, width: `${layout.rosterTitleWidth * 100}%`, height: `${Math.max(0.022, layout.rosterTitleSize * 1.4) * 100}%`, fontSize: `${layout.rosterTitleSize * 100}cqw`, transform: "translate(-50%,-50%)" }}><span className="translate-y-[0.1em]">{players.length ? "ELENCO CAMPEÃO" : "TÍTULO CONQUISTADO"}</span></button>
