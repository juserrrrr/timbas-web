...
export default async function HistoryPage({ searchParams }: { searchParams: Promise<{ page?: string }> }) {
  ...
  const [history, detail, duoData] = await Promise.all([
    fetchMatchHistory(token, serverId, page, 10).catch(() => ({ data: [], total: 0, page: 1, pages: 1, hasNext: false })),
    fetchPlayerDetailStats(token, serverId, userId).catch(() => null),
    fetchDuoStats(token, serverId, userId).catch(() => ({ partners: [], opponents: [] })),
  ])
    ...
    <div className="space-y-6 animate-in fade-in duration-700">
      ...
      {matches.length === 0 && page === 1 ? (
        ...
        <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-500">
          {/* Side stats */}
          {detail && (detail.blueSide.total > 0 || detail.redSide.total > 0) && (
          ...
          {detail && detail.matchTypeStats.length > 0 && (
              ...
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                {detail.matchTypeStats.map((t) => (
                  <div key={t.type} className="rounded-xl bg-black/40 p-4 ring-1 ring-white/[0.05]">
                    <p className="text-xs text-gray-500 mb-1 font-medium">{t.label}</p>
                    <p className={`text-2xl font-black ${t.winRate >= 0.5 ? "text-green-400" : "text-red-400"}`}>
                      {Math.round(t.winRate * 100)}%
                    </p>
                    <p className="text-xs text-gray-400 mt-1 font-medium">{t.wins}V / {t.losses}D em {t.total}</p>
                  </div>
                ))}
              </div>
            </Card>
          ...
          <div className="space-y-4">
            {matches.map((match, index) => {
              ...
              const matchNum = history.total - ((page - 1) * 10) - index
              ...
              return (
                ...
                >
                  ...
                  {pending ? (
                    <div className="mb-6 flex flex-col items-center gap-3 border-b border-white/[0.06] pb-4 text-center">
                      <div className="flex h-12 w-12 items-center justify-center rounded-xl border border-white/[0.1] bg-gradient-to-br from-blue-600/20 to-red-600/20 ring-1 ring-white/5">
                        <span className="text-xl font-black text-white">#{matchNum}</span>
                      </div>
                      <div>
                        <h3 className="text-lg font-black text-white">Partida {matchNum}</h3>
                    ...
                    <div className="mb-6 flex flex-wrap items-center justify-between gap-4 border-b border-white/[0.06] pb-4">
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {myTeamId !== null && (
                          <Badge className={iWon ? "bg-green-500/20 text-green-400 border-green-500/50 font-bold" : "bg-red-500/20 text-red-400 border-red-500/50 font-bold"}>
                  ...
                  <div className="grid gap-6 md:grid-cols-2 md:items-stretch">
                    <div className={`flex flex-col rounded-xl border p-4 transition-colors ${blueWon ? "border-blue-500/30 bg-blue-500/10" : "border-white/[0.06] bg-black/40"} ${inBlue ? "ring-1 ring-blue-500/30" : ""}`}>
                      ...
                      <div className="space-y-1.5">
                        {match.blueTeam.players.map((player) => (
                          <div key={player.userId} className={`flex items-center justify-between rounded-lg px-3 py-2 text-sm transition-colors ${player.userId === userId ? "bg-blue-500/20 ring-1 ring-blue-500/30" : "bg-black/20"}`}>
                            <span className={`font-bold ${player.userId === userId ? "text-blue-200" : "text-gray-300"}`}>{player.name}</span>
                    ...
                    <div className={`flex flex-col rounded-xl border p-4 transition-colors ${redWon ? "border-red-500/30 bg-red-500/10" : "border-white/[0.06] bg-black/40"} ${inRed ? "ring-1 ring-red-500/30" : ""}`}>
                      <div className="mb-3 flex min-h-[28px] items-center justify-between">
                        <div className="flex items-center gap-2">
                      ...
                      <div className="space-y-1.5">
                        {match.redTeam.players.map((player) => (
                          <div key={player.userId} className={`flex items-center justify-between rounded-lg px-3 py-2 text-sm transition-colors ${player.userId === userId ? "bg-red-500/20 ring-1 ring-red-500/30" : "bg-black/20"}`}>
                            <span className={`font-bold ${player.userId === userId ? "text-red-200" : "text-gray-300"}`}>{player.name}</span>
                            {player.position && <span className="text-[10px] font-bold uppercase tracking-wider text-gray-600">{player.position}</span>}
                          </div>
                        ))}
                      </div>
          ...
          <HistoryPagination page={history.page} pages={history.pages} total={history.total} />
        </div>
      )}
    </div>
  )
