...
export default function EaClubPlayerPage() {
  ...
  return <div className="mx-auto max-w-7xl space-y-6">
    ...
    <section className="space-y-4">
      <div><p className="text-xs font-bold uppercase tracking-[0.16em] text-blue-400">Análise por posição</p><h2 className="text-xl font-black text-white">Onde rende melhor</h2><p className="mt-1 text-sm text-gray-500">Análise agregada das atuações capturadas, sem exibir o histórico das partidas.</p></div>
      {player.positionAnalysis.length > 0 ? <>
        <div className="grid gap-3 sm:grid-cols-2">
          <Card className="border-blue-500/15 bg-blue-500/[0.05] p-5"><p className="text-xs font-bold uppercase tracking-wider text-blue-400">Posição mais jogada</p><p className="mt-2 text-2xl font-black text-white">{positionName(player.mostPlayedPosition)}</p><p className="mt-1 text-sm text-gray-500">{player.positionAnalysis.find((item) => item.position === player.mostPlayedPosition)?.appearances ?? 0} atuações registradas</p></Card>
