...
export default function EaClubPlayerPage() {
  ...
  const career = player.careerGames == null ? [] : [
    { label: "Partidas", value: player.careerGames },
    { label: "Gols", value: player.careerGoals },
    { label: "Assistências", value: player.careerAssists },
    { label: "MVPs", value: player.careerMvps },
    { label: "Média", value: display(player.careerRating, 1) },
  ]
  return <div className="space-y-6"><ClubPageHeader name={player.playerName} subtitle={`Carreira no ${club?.nickname || club?.name || "clube"}`} />{career.length > 0 && <Card className="border-emerald-500/15 bg-emerald-500/[0.04] p-5"><div className="mb-4"><p className="text-xs font-bold uppercase tracking-[0.16em] text-emerald-400">Totais de carreira informados pela EA</p><p className="mt-1 text-xs text-gray-500">Podem incluir partidas anteriores ao início do monitoramento.</p></div><div className="grid grid-cols-2 gap-3 md:grid-cols-5">{career.map(item => <div key={item.label} className="rounded-xl border border-white/[0.06] bg-black/20 p-4 text-center"><p className="text-2xl font-black text-white">{item.value ?? "—"}</p><p className="text-xs text-gray-500">{item.label}</p></div>)}</div></Card>}<div><p className="mb-3 text-xs font-bold uppercase tracking-[0.16em] text-blue-400">Calculado pelas partidas armazenadas</p><div className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-5">{metrics.map(({ label, value, icon: Icon }) => <Card key={label} className="border-white/[0.07] bg-white/[0.025] p-4"><Icon className="mb-3 h-4 w-4 text-blue-400" /><p className="text-2xl font-black tabular-nums text-white">{value}</p><p className="text-xs text-gray-500">{label}</p></Card>)}</div></div><Card className="border-white/[0.07] bg-white/[0.025] p-5"><div className="mb-4 flex items-center gap-2"><Percent className="h-5 w-5 text-blue-400" /><h2 className="font-black text-white">Médias e eficiência</h2></div><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">{rates.map(rate => <div key={rate.label} className="rounded-xl border border-white/[0.06] bg-black/30 p-4"><p className="text-xl font-black text-white">{rate.value}</p><p className="mt-1 text-xs text-gray-500">{rate.label}</p></div>)}</div></Card></div>
}
