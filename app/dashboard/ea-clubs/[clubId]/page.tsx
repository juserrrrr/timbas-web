...
export default function EaClubDashboardPage({ params }: { params: Promise<{ clubId: string }> }) {
  ...
  return <div className="mx-auto max-w-7xl space-y-8">
    ...
    <section>
      <div className="mb-4">
        <p className="text-xs font-bold uppercase tracking-[0.16em] text-blue-400">Destaques do elenco</p>
        <h2 className="text-xl font-black text-white">Os donos do jogo</h2>
      </div>
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
        {HIGHLIGHTS.map((highlight) => {
          const category = categories.find((item) => item.key === highlight.key)
          const entry = category?.entries[0]
          const Icon = highlight.icon
          const isRating = highlight.key === "eaClubRating" || highlight.key === "defenders"
          const clubAppearances = entry
            ? categories.find((item) => item.key === "eaClubGames")?.entries.find((item) => item.player.id === entry.player.id)?.value
            : undefined
          const appearances = clubAppearances ?? entry?.appearances

          return <Card key={highlight.key} className="relative overflow-hidden border-white/[0.07] bg-white/[0.025] p-5">
            <div className="flex items-start justify-between">
              <div className={`rounded-xl bg-white/[0.04] p-3 ${highlight.color}`}><Icon className="h-5 w-5" /></div>
            ...
            </Link> : <p className="mt-2 text-sm text-gray-600">Sem dados disponíveis</p>}
          </Card>
        })}
      </div>
    </section>
