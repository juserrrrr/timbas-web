...
/**
 * SFU transport: the host publishes one copy to the LiveKit server and the
 * server fans it out. Upload and CPU stop growing with the audience, and the
 * room has no practical size limit.
 *
 * The capture side is identical to the peer to peer path because it lives in
 * useLiveMedia. All that changes here is where the two tracks are sent.
 */
export function useSfuBroadcast(
  streamId: string,
  peerId: string,
