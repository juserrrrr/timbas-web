...
function StatRow({ label, left, right, winner, formatFn = (v: number) => String(v) }: { label: string; left: number; right: number; winner: Winner; formatFn?: (v: number) => string }) {
  ...
  return (
    <div className="space-y-1.5">
      ...
      <div className="flex h-1.5 overflow-hidden rounded-full bg-gray-800">
        ...
        <div className={`h-full rounded-r-full transition-all duration-500 ${winner === "right" ? "bg-red-500" : "bg-red-500/30"}`} style={{ width: `${rightPct}%` }} />
      </div>
    </div>
  )
}
...
function RecentForm({ form }: { form: ("W" | "L")[] }) {
  ...
  return (
    <div className="flex flex-wrap gap-1.5">
      {form.map((r, i) => (
        <span key={i} className={`flex h-8 w-8 items-center justify-center rounded-md text-xs font-bold ${r === "W" ? "bg-green-500/20 text-green-400 ring-1 ring-green-500/30" : "bg-red-500/20 text-red-400 ring-1 ring-red-500/30"}`}>
          {r === "W" ? "V" : "D"}
        </span>
      ))}
    </div>
...
function SideBar({ wins, losses, total, winRate, color, label }: { wins: number; losses: number; total: number; winRate: number; color: "blue" | "red"; label: string }) {
  const pct = Math.round(winRate * 100)
  return (
    <div className="space-y-1">
      <div className="flex justify-between text-xs">
        <span className={`flex items-center gap-1.5 ${color === "blue" ? "text-blue-400" : "text-red-400"}`}>
          <span className={`inline-block h-1.5 w-1.5 rounded-full ${color === "blue" ? "bg-blue-500" : "bg-red-500"}`} />{label}
        </span>
        <span className="text-gray-300 tabular-nums">{wins}V / {losses}D <span className="text-gray-500">({pct}%)</span></span>
      </div>
      <div className="h-1.5 overflow-hidden rounded-full bg-gray-800">
        <div className={`h-full rounded-full transition-all ${color === "blue" ? "bg-blue-500" : "bg-red-500"}`} style={{ width: `${pct}%` }} />
      </div>
...
export function VersusClient({ players, serverId }: Props) {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    setError(null)
    setLeftPlayer(players.find((p) => String(p.userId) === leftId) ?? null)
    setRightPlayer(players.find((p) => String(p.userId) === rightId) ?? null)

    Promise.all([
      apiFetch(`${process.env.NEXT_PUBLIC_API_URL}/leaderboard/${serverId}/player/${leftId}`, { headers: authHeaders(token) }).then((r) => r.json()),
      apiFetch(`${process.env.NEXT_PUBLIC_API_URL}/leaderboard/${serverId}/player/${rightId}`, { headers: authHeaders(token) }).then((r) => r.json()),
    ])
      .then(([ld, rd]: [PlayerDetailStats, PlayerDetailStats]) => { setLeftDetail(ld); setRightDetail(rd) })
      .catch(() => setError("Falha ao carregar estatísticas."))
      .finally(() => setLoading(false))
  }, [leftId, rightId, serverId])
      ...
      <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-4">
        <Select value={leftId} onValueChange={setLeftId} disabled={players.length === 0}>
          ...
          <SelectContent className="border-gray-700 bg-gray-900 text-white">
            {players.map((p) => (
              <SelectItem key={p.userId} value={String(p.userId)} disabled={String(p.userId) === rightId} className="focus:bg-gray-800 focus:text-white">
                {p.rank}º {p.name}
              </SelectItem>
            ))}
          </SelectContent>
        ...
        <Select value={rightId} onValueChange={setRightId} disabled={players.length === 0}>
          ...
          <SelectContent className="border-gray-700 bg-gray-900 text-white">
            {players.map((p) => (
              <SelectItem key={p.userId} value={String(p.userId)} disabled={String(p.userId) === leftId} className="focus:bg-gray-800 focus:text-white">
                {p.rank}º {p.name}
              </SelectItem>
            ))}
          </SelectContent>
            ...
            <Card className={`border p-5 text-center backdrop-blur-sm transition-all ${winner === "left" ? "border-blue-500/40 bg-blue-500/10 shadow-lg shadow-blue-500/10" : "border-gray-800/50 bg-gray-900/50"}`}>
              <div className="flex flex-col items-center gap-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {winner === "left" && <span className="rounded-full bg-blue-500/20 px-2 py-0.5 text-xs font-semibold text-blue-400 ring-1 ring-blue-500/30">Melhor WR</span>}
                </div>
              </div>
            </Card>
            ...
            <Card className={`border p-5 text-center backdrop-blur-sm transition-all ${winner === "right" ? "border-red-500/40 bg-red-500/10 shadow-lg shadow-red-500/10" : "border-gray-800/50 bg-gray-900/50"}`}>
              <div className="flex flex-col items-center gap-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {winner === "right" && <span className="rounded-full bg-red-500/20 px-2 py-0.5 text-xs font-semibold text-red-400 ring-1 ring-red-500/30">Melhor WR</span>}
                </div>
              </div>
            </Card>
          </div>
