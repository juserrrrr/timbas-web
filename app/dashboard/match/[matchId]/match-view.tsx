...
export function MatchView({
  ...
  matchId: number
  initialMatch: CustomLeagueMatch | null
  initialError: string | null
}) {
  const [match, setMatch] = useState<CustomLeagueMatch | null>(initialMatch)
  ...
  // ── SSE connection ──────────────────────────────────────────────────────
  useEffect(() => {
    if (isNaN(matchId) || !token) return
    let es: EventSource
    let cancelled = false

    apiFetch(`${process.env.NEXT_PUBLIC_API_URL}/leagueMatch/${matchId}/events/ticket`, {
      method: 'POST',
      ...
      .then(({ ticket }) => {
        if (cancelled) return
        const url = `${getMatchEventsUrl(String(matchId))}?ticket=${encodeURIComponent(ticket)}`
  ...
  const handleJoin = () => runAction("join", () => joinMatch(token!, matchId, me!.discordId!))
  const handleLeave = () => runAction("leave", () => leaveMatch(token!, matchId, me!.discordId!))
  const handleDraw = () => runAction("draw", () => drawTeams(token!, matchId, me!.discordId!))
  const handleStart = () => runAction("start", () => startMatch(token!, matchId, me!.discordId!))

  const handleFinish = (winnerSide: "BLUE" | "RED") => {
    setShowFinishModal(false)
    runAction("finish", () => finishMatch(token!, matchId, me!.discordId!, winnerSide))
  }
  ...
  const confirmCancel = () => {
    ...
    runAction("cancel", () => cancelMatch(token!, matchId, me!.discordId!))
  }
  ...
  const confirmKick = () => {
    ...
    setKickTarget(null)
    runAction("kick", () => kickPlayer(token!, matchId, me!.discordId!, discordId))
  }
  ...
  const handleVoiceMove = async () => {
    ...
    try {
      await moveToRoom(token, matchId)
    } catch (e: any) {
      ...
      <div className="relative px-4 py-6 sm:px-8 lg:py-8">
        ...
        {match.status !== "EXPIRED" && (
          <div className="flex flex-wrap items-center gap-3 animate-in fade-in duration-500">
            {token && canJoin && (
              <ActionBtn id="btn-join" icon={<LogIn className="h-4 w-4" />} label="Entrar" color="green" loading={actionLoading === "join"} onClick={handleJoin} />
            )}
            {token && canLeave && (
              <ActionBtn id="btn-leave" icon={<LogOut className="h-4 w-4" />} label="Sair" color="gray" loading={actionLoading === "leave"} onClick={handleLeave} />
            )}
            ...
            {canFinish && (
              <ActionBtn id="btn-finish" icon={<Flag className="h-4 w-4" />} label="Finalizar" color="red" loading={actionLoading === "finish"} onClick={() => setShowFinishModal(true)} />
            )}
            {isCreator && match.status !== "FINISHED" && match.status !== "EXPIRED" && (
              <ActionBtn id="btn-cancel" icon={<Trash className="h-4 w-4" />} label="Encerrar" color="red" loading={actionLoading === "cancel"} onClick={handleCancel} />
            )}
            {match.status === "WAITING" && !match.teamBlueId && (
              <div className="ml-auto rounded-full border border-white/[0.06] bg-white/[0.03] px-3 py-1.5 text-xs font-medium text-gray-500">
                {qPlayers.length >= maxPlayers ? "✅ Pronto para sortear / iniciar" : `Aguardando mais ${maxPlayers - qPlayers.length} jogador${maxPlayers - qPlayers.length !== 1 ? "es" : ""}`}
              </div>
            )}
            {match.status === "WAITING" && match.teamBlueId && (
              <div className="ml-auto rounded-full border border-white/[0.06] bg-white/[0.03] px-3 py-1.5 text-xs font-medium text-gray-500">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
          <div className="mx-4 w-full max-w-sm rounded-2xl border border-white/[0.08] bg-[#0d0d14] p-6 shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="flex gap-3">
              <button onClick={() => setShowCancelModal(false)} className="flex-1 rounded-xl border border-white/10 bg-white/5 py-2.5 text-sm font-semibold text-gray-300 transition-colors hover:bg-white/10 cursor-pointer">Voltar</button>
              <button onClick={confirmCancel} className="flex-1 rounded-xl bg-red-500 py-2.5 text-sm font-bold text-white transition-colors hover:bg-red-600 cursor-pointer">Encerrar</button>
            </div>
          </div>
        </div>
      )}
      ...
      {kickTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
          <div className="mx-4 w-full max-w-sm rounded-2xl border border-white/[0.08] bg-[#0d0d14] p-6 shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="flex gap-3">
              <button onClick={() => setKickTarget(null)} className="flex-1 rounded-xl border border-white/10 bg-white/5 py-2.5 text-sm font-semibold text-gray-300 transition-colors hover:bg-white/10 cursor-pointer">Voltar</button>
              <button onClick={confirmKick} className="flex-1 rounded-xl bg-red-500 py-2.5 text-sm font-bold text-white transition-colors hover:bg-red-600 cursor-pointer">Remover</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
          <div className="mx-4 w-full max-w-sm rounded-2xl border border-white/[0.08] bg-[#0d0d14] p-6 shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="grid grid-cols-2 gap-3">
              <button id="btn-win-blue" onClick={() => handleFinish("BLUE")} className="flex flex-col items-center gap-2 rounded-xl border border-blue-500/30 bg-blue-500/10 p-4 transition-all hover:border-blue-500/50 hover:bg-blue-500/20 cursor-pointer">
                <Shield className="h-6 w-6 text-blue-400" />
                <span className="font-bold text-blue-300">Time Azul</span>
              </button>
              <button id="btn-win-red" onClick={() => handleFinish("RED")} className="flex flex-col items-center gap-2 rounded-xl border border-red-500/30 bg-red-500/10 p-4 transition-all hover:border-red-500/50 hover:bg-red-500/20 cursor-pointer">
                <Shield className="h-6 w-6 text-red-400" />
                <span className="font-bold text-red-300">Time Vermelho</span>
              </button>
            </div>
            <button onClick={() => setShowFinishModal(false)} className="mt-3 w-full rounded-xl py-2.5 text-sm text-gray-500 transition-colors hover:text-gray-300 cursor-pointer">Cancelar</button>
          </div>
        </div>
      )}
    </div>
