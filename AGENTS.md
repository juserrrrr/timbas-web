...
└── admin/                  # Admin-only area (role: ADMIN)
    ...
    ├── ai/                 # Pick provider and model per AI feature
    └── lab/                # Generate throwaway competitions to inspect the UI
...
Draft leagues are **created and managed only from `/admin`**. The dashboard is
where people join, draft, set a lineup, play rounds and trade; it must never grow
...
## Admin area
`/admin` holds everything that changes state for everyone: competitions, the
draft league panel, the player catalog, AI settings and the demo lab. Anything
generated by the lab is prefixed `[DEMO]`, pays no coins, and is removable in one
click.

## Writing
Portuguese, and **never an em dash**. Break the sentence in two, or use a comma,
colon or period. Empty values render as a plain `-`. Keep the tone direct and
human: say what the thing does for the person reading, not what the system does
internally.
